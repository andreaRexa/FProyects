...
<div class="container">
    <div class="card mt-4">
        <form action="{{ isset($proyecto) ? route('proyectos.update', $proyecto->id) : route('subirproyectos.subir') }}" method="POST" enctype="multipart/form-data" id="form-subir-proyecto">
            @csrf
            @if(isset($proyecto))
                @method('POST')
            @endif
